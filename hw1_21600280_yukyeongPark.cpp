...
typedef struct heap{
    ...
    int data_num; //현재 heap에 있는 데이터갯수
} Heap;
...
//going down and comparing and swapping values in heap
void down(Heap* heap, int index){
    while (index!= heap->data_num && heap->data[parent_idx(index)].score < heap->data[index].score){
        swap(heap, parent_idx(index), index);
        index=(index); //index위치가 parent idx로
    }
}
...
Element delete_heap(Heap* heap){
    //delete the first element of heap
